...
import { CareerSpell, PlayingLevels, WikipediaPlayerDetailsFields } from './types';
...
const storeClubDetails = async (clubName: string, clubUrl?: string) => {
  try {
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      if (error.code === 'P2002') {
        console.log(`ℹ️ Club ${clubName} already exists in database. Skipping.`)
      }
    } else {
...
const getPlayerDetails = async (playerUrl: string) => {
  ...
  try {
    ...
    const playerName = playerTable.find('.infobox-title').contents().first().text().trim()
    ...
    const playerDetails = rawTableRows.reduce((playerDetails, row) => {
      ...
      if (mapWikipediaFieldToKey[rowTitle as WikipediaPlayerDetailsFields]) {
        return {
          ...playerDetails,
          [mapWikipediaFieldToKey[rowTitle as WikipediaPlayerDetailsFields]]: removeParenthesisedText(rowValue)
        }
      } 

      return playerDetails
    }, {} as Omit<Prisma.PlayersCreateInput, 'name' | 'wikipediaUrl' | 'imageUrl'>)
    ...
    const youthCareerIndex = rawTableRows.indexOf(rawTableRows
      .find(row => $(row).find('th').text().includes('Youth career')) as cheerio.Element
    )
    const seniorCareerIndex = rawTableRows.indexOf(rawTableRows
      .find(row => $(row).find('th').text().includes('Senior career')) as cheerio.Element
    )
    const internationalCareerIndex = rawTableRows.indexOf(rawTableRows
      .find(row => $(row).find('th').text().includes('International career')) as cheerio.Element
    )
    const medalRecordIndex = rawTableRows.indexOf(rawTableRows
      .find(row => $(row).find('th').text().includes('Medal record')) as cheerio.Element
    )
    ...
    const youthCareerRows = youthCareerIndex !== -1 ? rawTableRows.slice(youthCareerIndex + 1, seniorCareerIndex) : []
    // Slice the first row as it is the header row
    const seniorCareerRows = rawTableRows.slice(seniorCareerIndex + 1, getSeniorEndIndex())
    const internationalCareerRows = internationalCareerIndex !== -1 
      ? rawTableRows.slice(
    ...
    const isValidCareerRow = (data: CareerSpell | undefined): data is CareerSpell => {
      // Explicitly check apps and goals are not null as !!0 will return false
      return !!data && !!data.clubId && !!data.from && data.apps !== null && data.goals !== null
    }

    const formatCareer = async (row: cheerio.Element, playingLevel: PlayingLevels) => {
      const isYouthCareer = playingLevel === PlayingLevels.YOUTH
      const clubCellSelector = isYouthCareer ? '.infobox-data' : '.infobox-data-a'
      const rowTitle = $(row).find('th').text().trim()
      ...
      const clubTitle = clubLinkTitle
      ? clubLinkTitle 
      : clubCellContent
      const loan = clubCellContent.includes('(loan)')
      const formattedClubTitle = loan 
        ? clubTitle.replace('→', '').replace('(loan)', '').replace(' national football team', '').trim() 
        : clubTitle
      ...
      const clubId = scrapedClubLinkList[`${formattedClubTitle}-${clubLinkHref}`.toLowerCase()] 
    ...
    const seniorCareer = await Promise.all(
        ...
        data.filter((data) => isValidCareerRow(data)).filter(Boolean) as CareerSpell[]
      ) 
    const internationalCareer = await Promise.all(
        internationalCareerRows.map((row) => formatCareer(row, PlayingLevels.INTERNATIONAL))
      ).then((data) => 
        data.filter((data) => isValidCareerRow(data)).filter(Boolean) as CareerSpell[]
      ) 
...
const scrapeTransferHistory = async (url: string) => {
  try {
    ...
    categoryGroups.each((_index, group) => {
      $(group).find('a').each((_index, player) => {
